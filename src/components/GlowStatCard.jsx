...
const SVG_ICONS = {
  ...
  check:    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="9 12 11 14 15 10"/></svg>,
  users:    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="7" r="4"/><path d="M3 21v-2a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v2"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/><path d="M21 21v-2a4 4 0 0 0-3-3.87"/></svg>,
  ...
  userx:    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="17" y1="8" x2="23" y2="14"/><line x1="23" y1="8" x2="17" y2="14"/></svg>,
...
export default function GlowStatCard({ icon, label, value, sub, palette = 'blue', trend }) {
  ...
  function handleEnter() { if (mobile) return; setLeaving(false); setHovered(true) }
  function handleLeave() { if (mobile) return; setHovered(false); setLeaving(true); setTimeout(() => setLeaving(false), 400) }

  const glowBase  = `0 4px 20px rgba(${p.glowColor},.28), 0 1px 4px rgba(0,0,0,.07)`
  const glowHover = `0 8px 36px rgba(${p.glowColor},.60), 0 2px 10px rgba(0,0,0,.12), 0 0 0 1.5px rgba(${p.glowColor},.22)`

  const animation = !mobile && (hovered
    ? 'sc-bounce 0.45s cubic-bezier(.34,1.56,.64,1) forwards'
    : leaving ? 'sc-out 0.35s cubic-bezier(.4,0,.2,1) forwards' : 'none')

  /* ── MOBILE: card piatta e compatta, layout riga ── */
  if (mobile) {
    return (
      <div style={{
        ...
        alignItems: 'center',
        gap: '10px',
        boxShadow: `0 2px 10px rgba(${p.glowColor},.22)`,
        minWidth: 0,
      }}>
        {/* Icona piccola */}
        <div style={{
        ...
        {/* Trend badge opzionale */}
        {trend != null && (
          <div style={{
            fontSize: '10px', fontWeight: '700',
            backgroundColor: p.badge, color: '#fff',
  ...
  /* ── DESKTOP: card piena con effetti ── */
  return (
    <>
      <style>{BOUNCE}</style>
      <div
        ...
        style={{
          ...
          animation, transition: 'box-shadow 0.22s ease',
        }}
      >
        {/* Cerchi decorativi */}
        <div style={{ position:'absolute', right:'-16px', top:'-16px', width:'88px', height:'88px', borderRadius:'50%', backgroundColor: hovered ? 'rgba(255,255,255,.16)' : 'rgba(255,255,255,.10)', transition:'background-color 0.25s', pointerEvents:'none' }}/>
        <div style={{ position:'absolute', right:'18px', bottom:'-22px', width:'56px', height:'56px', borderRadius:'50%', backgroundColor: hovered ? 'rgba(255,255,255,.12)' : 'rgba(255,255,255,.07)', transition:'background-color 0.25s', pointerEvents:'none' }}/>
        {/* Shimmer */}
        <div style={{ position:'absolute', inset:0, background: hovered ? 'linear-gradient(135deg, rgba(255,255,255,.10) 0%, rgba(255,255,255,0) 60%)' : 'none', borderRadius:'12px', pointerEvents:'none', transition:'opacity 0.25s' }}/>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', position:'relative' }}>
          ...
          {trend != null && (
            <div style={{ fontSize:'11px', fontWeight:'700', backgroundColor: p.badge, color:'#fff', padding:'3px 8px', borderRadius:'20px', whiteSpace:'nowrap' }}>
        ...
        <div style={{ position:'relative' }}>
          <p style={{
            fontSize:'36px', fontWeight:'900', color: p.num, margin:0,
            letterSpacing:'-0.04em', lineHeight:1, fontFamily:"'Inter',sans-serif",
          ...
          <p style={{ fontSize:'11px', fontWeight:'700', color: p.label, margin:'6px 0 0', textTransform:'uppercase', letterSpacing:'0.05em', fontFamily:"'Inter',sans-serif" }}>
            {label}
          </p>
          {sub && <p style={{ fontSize:'12px', color: p.sub, margin:'3px 0 0', fontFamily:"'Inter',sans-serif" }}>{sub}</p>}
        </div>
      </div>
    </>
  )
