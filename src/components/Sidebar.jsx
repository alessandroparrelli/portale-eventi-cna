...
import {
  ...
  QrCode, BarChart2, LogOut, ChevronRight, UserCog,
} from 'lucide-react'
      ...
      <div style={styles.logoArea}>
        {!collapsed && (
          <img src="https://raw.githubusercontent.com/alessandroparrelli/fileappoggio/main/NUOVO-LOGO-CNA-ROMA-SOLO-ROMA.png"
            alt="CNA Roma" style={styles.logo}/>
        )}
      ...
      <div style={styles.bottomArea}>
        {!collapsed && (
          <div style={styles.userInfo}>
            <div style={{ ...styles.userAvatar, backgroundColor: RUOLO_COLORS[ruolo] || '#003DA5' }}>
              {username.charAt(0).toUpperCase()}
            </div>
            <div style={{ minWidth:0 }}>
              <p style={styles.userName}>{username}</p>
              <p style={{ ...styles.userRole, color: RUOLO_COLORS[ruolo] || '#6B7280', textTransform:'capitalize' }}>{ruolo}</p>
            </div>
          </div>
        )}
...
const styles = {
  sidebar: { height:'100vh', backgroundColor:'#FFFFFF', borderRight:'1px solid #E5E7EB', display:'flex', flexDirection:'column', position:'fixed', top:0, left:0, zIndex:100, transition:'width 0.2s ease', overflow:'hidden' },
  logoArea: { display:'flex', alignItems:'center', padding:'20px 16px', borderBottom:'1px solid #E5E7EB', minHeight:'80px', gap:'8px' },
  logo: { height:'40px', objectFit:'contain', flex:1, minWidth:0 },
  collapseBtn: { background:'none', border:'1px solid #E5E7EB', borderRadius:'4px', cursor:'pointer', padding:'4px', color:'#6B7280', display:'flex', alignItems:'center', flexShrink:0 },
  nav: { flex:1, padding:'12px 8px', display:'flex', flexDirection:'column', gap:'2px', overflowY:'auto' },
  navLink: { display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', borderRadius:'4px', textDecoration:'none', fontSize:'14px', letterSpacing:'-0.01em', transition:'background-color 0.15s, color 0.15s', whiteSpace:'nowrap' },
  ...
  userAvatar: { width:'32px', height:'32px', borderRadius:'50%', color:'#FFFFFF', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'14px', fontWeight:'700', flexShrink:0 },
  userName: { fontSize:'13px', fontWeight:'600', color:'#0A0A0A', margin:0, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' },
  userRole: { fontSize:'11px', margin:0, fontWeight:'600' },
  logoutBtn: { display:'flex', alignItems:'center', gap:'8px', padding:'10px 12px', background:'none', border:'none', cursor:'pointer', fontSize:'14px', fontFamily:"'Inter',sans-serif", color:'#DC2626', fontWeight:'500', borderRadius:'4px', width:'100%', transition:'background-color 0.15s' },
}
