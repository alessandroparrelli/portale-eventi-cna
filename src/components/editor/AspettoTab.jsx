    ...
    <div style={{ border: '1px solid #E5E7EB', borderRadius: '10px', overflow: 'hidden', marginBottom: '10px' }}>
      <button
        ...
        style={{
          ...
          padding: '13px 16px', background: open ? '#EBF0FA' : '#FAFAFA',
...
const PALETTE = [
  ...
  { nome: 'CNA Pallini',  primario: '#003DA5', pulsanti: '#E8792F', sfondo_header: '#FFFFFF', bordo_header: '#003DA5', sfondo_pagina: '#FFFFFF', sfondo_sezioni: '#F8F9FF', cta_bg: '#EBF0FA', colore_testo_btn: '#FFFFFF', sfondo_pattern: 'pallini_angolo', pattern_colore1: '#003DA5', pattern_colore2: '#E8792F', pattern_opacita: '25' },
]
            ...
            <div style={{ display: 'flex', gap: '4px', marginTop: '6px' }}>
              {[['24', 'XS'], ['44', 'S'], ['64', 'M'], ['96', 'L'], ['128', 'XL'], ['160', 'XXL']].map(([v, l]) => (
                <button key={v} type="button" onClick={() => setT('logo_altezza', v)}
                  style={{ flex: 1, padding: '4px', border: `1px solid ${tema.logo_altezza == v ? '#003DA5' : '#E5E7EB'}`, borderRadius: '5px', background: tema.logo_altezza == v ? '#EBF0FA' : '#fff', fontSize: '11px', fontWeight: '700', color: tema.logo_altezza == v ? '#003DA5' : '#6B7280', cursor: 'pointer', fontFamily: "'Outfit',sans-serif" }}
                >{l}</button>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>
              {[['trasparente', 'Trasparente'], ['bianco', 'Bianco'], ['colore_primario', 'Colore brand']].map(([v, l]) => (
                <button key={v} type="button" onClick={() => setT('logo_bg', v)}
                  style={{ flex: 1, padding: '8px 6px', border: `1px solid ${tema.logo_bg === v ? '#003DA5' : '#E5E7EB'}`, borderRadius: '7px', background: tema.logo_bg === v ? '#EBF0FA' : '#fff', fontSize: '11px', fontWeight: '700', color: tema.logo_bg === v ? '#003DA5' : '#6B7280', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", textAlign: 'center' }}
                >{l}</button>
              ))}
            </div>
          </div>
