  ...
  return (
    <div style={{ padding:'20px 16px', maxWidth:'1000px', fontFamily:'Inter,sans-serif' }}>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)}
        ...
        style={{ width:'100%', boxSizing:'border-box', padding:'10px 14px', border:'1px solid #E8ECF4', borderRadius:'16px', fontSize:'14px', fontFamily:'Inter,sans-serif', outline:'none', marginBottom:'14px' }}
      />
      ...
      {/* Tabella desktop */}
      {!loading && filtered.length > 0 && (
        <div className="lp-table-wrap" style={{ background:'#fff', borderRadius:'16px', border:'1px solid #E8ECF4', overflow:'hidden' }}>
      ...
      {!loading && filtered.length > 0 && (
        <div className="lp-card-list" style={{ flexDirection:'column', gap:'10px' }}>
          {filtered.map(p => {
            ...
            const n  = counts[p.id] || 0
            return (
              <div key={p.id} style={{ background:'#fff', border:'1px solid #E8ECF4', borderRadius:'16px', padding:'14px 16px' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <div style={{ display:'flex', gap:'16px', fontSize:'12px', color:'#9CA3AF' }}>
                    ...
                    {n > 0 && <span style={{ color:'#5B5FEF', fontWeight:'700' }}>{n} contatti</span>}
                  </div>
