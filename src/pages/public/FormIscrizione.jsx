  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
      ...
      <input type={type} value={value} onChange={onChange} placeholder={placeholder}
        style={{ padding:'10px 12px', border:`1px solid ${error?'#DC2626':'#D1D5DB'}`,
...
export default function FormIscrizione({ event, onSuccess }) {
  ...
  async function submit(e) {
    ...
    setLoading(false)
    // Passa i dati completi alla modale di conferma nel parent
    onSuccess?.({
      id:       reg.id,
      qr_code:  reg.qr_code,
      nome:     form.nome.trim(),
        ...
        <div key={f.id} style={{ marginTop:'12px', display:'flex', flexDirection:'column', gap:'4px' }}>
          ...
          {f.tipo === 'select' ? (
            <select value={extras[f.id]||''} onChange={e => setExtras(p => ({...p,[f.id]:e.target.value}))}
              style={{ padding:'10px 12px', border:`1px solid ${errors[`extra_${f.id}`]?'#DC2626':'#D1D5DB'}`,
                borderRadius:'6px', fontSize:'14px', fontFamily:"'Inter',sans-serif", outline:'none' }}>
              <option value="">— Seleziona —</option>
            ...
            <input
              type={f.tipo==='email'?'email':f.tipo==='telefono'?'tel':f.tipo==='numero'?'number':f.tipo==='data'?'date':'text'}
...
const s = {
  ...
  errBox: { backgroundColor:'#FEF2F2', border:'1px solid #FECACA', borderRadius:'6px',
    padding:'10px 14px', fontSize:'14px', color:'#DC2626', marginBottom:'12px' },
  submitBtn: { marginTop:'20px', display:'flex', alignItems:'center', justifyContent:'center',
    gap:'8px', backgroundColor:'#003DA5', color:'#FFFFFF', border:'none', borderRadius:'6px',
    padding:'14px 24px', fontSize:'15px', fontWeight:'700', fontFamily:"'Inter',sans-serif",
